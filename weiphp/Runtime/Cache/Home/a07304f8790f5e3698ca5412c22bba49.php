      ...
      <ul class="sidenav">
        <li>
          <?php if(!empty($now_top_menu_name)): ?><a class="sidenav_parent" href="javascript:;"> 
            <!--<img src="/weiphp/Public/Home/images/left_icon_<?php echo ($core_side_category["left_icon"]); ?>.png"/>--> 
...
<?php if(!empty($normal_tips)): ?><p class="normal_tips"><b class="fa fa-info-circle"></b> <?php echo ($normal_tips); ?></p><?php endif; ?>
      <div class="table-bar">
        <div class="fl">
          <div class="tools"> <a href="javascript:void(0);" class="btn setting_group">设置用户组</a> &nbsp;<a href="javascript:void(0);" class="btn setting_tag">设置标签</a> &nbsp; <a href="<?php echo U ( 'Home/AuthGroup/export' );?>" class="btn">导出用户</a> &nbsp; 
          <?php if($syc_wechat): ?><a href="<?php echo U('syc_auth_group');?>" class="btn tongbu">一键同步微信公众号粉丝</a> &nbsp;<?php endif; ?> 
          </div>
        </div>
        <!-- 高级搜索 -->
        <div class="search-form fr cf">
          ...
          <div class="sleft">
            <input type="text" placeholder="请输入昵称或标签搜索" value="<?php echo I('nickname');?>" class="search-input" name="nickname">
  ...
  $("#search").click(function(){
    ...
    //用户标签弹框
		var html = $('<div class="manage_group normal_dialog">'+
                  '<div class="content">'+
                          '<?php if(is_array($tags)): $i = 0; $__LIST__ = $tags;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>'+
                            '<div class="check-item"><input type="checkbox" class="regular-checkbox select_tag" value="<?php echo ($vo["id"]); ?>" id="tag_<?php echo ($vo["id"]); ?>" name="tags[]">'+
                              '<label for="tag_<?php echo ($vo["id"]); ?>"></label><?php echo ($vo["title"]); ?> </div>'+
