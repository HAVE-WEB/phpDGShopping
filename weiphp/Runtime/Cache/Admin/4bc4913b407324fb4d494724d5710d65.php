					...
					<div class="form-item cf">
						...
						<div class="controls">
							<select name="addon">
                                ...
                                <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$m): $mod = ($i % 2 );++$i;?><option value="<?php echo ($key); ?>" <?php if($info[addon] == $key): ?>selected="selected"<?php endif; ?> ><?php echo ($m); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
							</select>
						</div>
					</div>                    
				</div>
					...
					<label class="item-label">表单显示排序<span class="check-tips">（直接拖动进行排序）</span></label>
						...
						<div class="form-item cf edit_sort edit_sort_l form_field_sort">
							...
							<ul class="dragsort needdragsort" data-group="<?php echo ($key); ?>">
								<?php if(is_array($fields)): foreach($fields as $k=>$field): if($field['is_show'] == 1): ?><li class="getSort">
											<em data="<?php echo ($field['id']); ?>"><?php echo ($field['title']); ?> [<?php echo ($field['name']); ?>]</em>
											<input type="hidden" name="field_sort[]" value="<?php echo ($field['name']); ?>"/>
